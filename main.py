...
def dailyclaim(token):
    ...
    try:
        ...
        if response_json.get("success"):
            ...
            log_message("Start Run Nodepay", Fore.YELLOW)
            return False
    except Exception as e:
...
def handle_logout(proxy):
    ...
    log_message("Logged out and cleared session info", Fore.RED)
...
async def start_ping(proxy, token):
    try:
        ...
        log_message(f"Ping task for proxy was cancelled", Fore.YELLOW)
    except Exception as e:
        log_message(f"Error in start_ping for proxy: {e}", Fore.RED)
...
async def ping(proxy, token):
    ...
    if proxy in last_ping_time and (current_time - last_ping_time[proxy]) < PING_INTERVAL:
        log_message(f"Skipping ping for proxy due to interval", Fore.YELLOW)
        return
    ...
    try:
        ...
        if response["code"] == 0:
            ip_score = response["data"].get("ip_score", "N/A")  # Extract IP score from response data
            log_message(f"✅ Connected = proxy 🌐 IP Score: {ip_score}", Fore.GREEN)
            RETRIES = 0
            status_connect = CONNECTION_STATES["CONNECTED"]
        else:
            handle_ping_fail(proxy, response)
    except Exception as e:
        log_message(f"Ping failed: {e}", Fore.RED)
...
async def call_api(url, data, proxy, token):
    ...
    try:
        ...
        log_message(f"Error during API call to {url}: {e}", Fore.RED)
...
async def render_profile_info(proxy, token):
    ...
    try:
        ...
        log_message(f"Error in render_profile_info: {e}", Fore.RED)
        error_message = str(e)
        if any(phrase in error_message for phrase in [
            "sent 1011 (internal error) keepalive ping timeout; no close frame received",
            "500 Internal Server Error"
        ]):
            log_message(f"Removing error proxy from the list", Fore.RED)
    ...
    while True:
        for token in tokens:
            ...
            if not active_proxies:
                log_message("No valid proxies available. Retrying...", Fore.RED)
                await asyncio.sleep(10)
                continue

            tasks = {asyncio.create_task(render_profile_info(proxy, token)): proxy for proxy in active_proxies}

            # Only call asyncio.wait if there are tasks to wait for
            if tasks:
                done, pending = await asyncio.wait(tasks.keys(), return_when=asyncio.FIRST_COMPLETED)
                for task in done:
                    failed_proxy = tasks[task]
                    if task.result() is None:
                        log_message(f"Removing and replacing failed proxy", Fore.RED)
